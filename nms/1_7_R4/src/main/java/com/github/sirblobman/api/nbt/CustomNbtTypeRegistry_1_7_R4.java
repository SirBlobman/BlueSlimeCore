...
public final class CustomNbtTypeRegistry_1_7_R4 {
    private final Function<Class<?>, CustomNbtTagAdapter<?, ?>> CREATE_ADAPTER = this::createAdapter;
    private final Map<Class<?>, CustomNbtTagAdapter<?, ?>> adapters = new HashMap<>();

    private <T> CustomNbtTagAdapter<?, ?> createAdapter(Class<T> type) {
    ...
    private <T, Z extends NBTBase> CustomNbtTagAdapter<T, Z> createAdapter(Class<T> primitiveType,
                                                                           Class<Z> nbtBaseType,
                                                                           Function<T, Z> builder,
                                                                           Function<Z, T> extractor) {
        return new CustomNbtTagAdapter<>(primitiveType, nbtBaseType, builder, extractor);
    }
    ...
    public <T> T extract(Class<T> type, NBTBase tag) throws ClassCastException, IllegalArgumentException {
        CustomNbtTagAdapter<?, ?> adapter = this.adapters.computeIfAbsent(type, this.CREATE_ADAPTER);
